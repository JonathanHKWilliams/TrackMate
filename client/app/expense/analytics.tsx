...
export default function AnalyticsScreen() {
  ...
  const renderTrends = () => {
    if (!analytics || analytics.trends.length === 0) return null;

    const maxAmount = Math.max(...analytics.trends.map((t) => t.amount));
    const minAmount = Math.min(...analytics.trends.map((t) => t.amount));
    const chartHeight = 160;
    const chartWidth = width - 72;
    const pointSpacing = chartWidth / (analytics.trends.length - 1 || 1);
        ...
        <View style={styles.chartContainer}>
          <View style={styles.lineChart}>
            {analytics.trends.map((trend, index) => {
              const pointHeight = maxAmount > 0 
                ? ((trend.amount - minAmount) / (maxAmount - minAmount || 1)) * chartHeight
                : chartHeight / 2;
              const yPosition = chartHeight - pointHeight;
              const xPosition = index * pointSpacing;
              const isHighest = trend.amount === maxAmount;
              const isLowest = trend.amount === minAmount && minAmount !== maxAmount;
                    ...
                    <View
                      style={[
                        styles.lineSegment,
                        {
                          ...
                          top: chartHeight - (maxAmount > 0 
                            ? ((analytics.trends[index - 1].amount - minAmount) / (maxAmount - minAmount || 1)) * chartHeight
                            : chartHeight / 2),
                          width: Math.sqrt(
                            Math.pow(pointSpacing, 2) + 
                            Math.pow(pointHeight - (maxAmount > 0 
                              ? ((analytics.trends[index - 1].amount - minAmount) / (maxAmount - minAmount || 1)) * chartHeight
                              : chartHeight / 2), 2)
                          ),
                          transform: [
                  ...
                  <View
                    style={[
                      styles.dataPoint,
                      {
                        ...
                        left: xPosition - 6,
                        top: yPosition - 6,
                        backgroundColor: isHighest ? '#FF8C00' : isLowest ? '#4CAF50' : '#FF8C00',
                        borderWidth: isHighest || isLowest ? 3 : 2,
                        borderColor: isHighest ? '#FFF' : isLowest ? '#FFF' : '#1A1A1A',
                      },
                    ]}
                  />
                  {(isHighest || isLowest || index === 0 || index === analytics.trends.length - 1) && (
                    <View
                      style={[
                        styles.valueLabel,
                        {
                    ...
                    >
                      <Text style={styles.valueLabelText}>${trend.amount.toFixed(0)}</Text>
                    </View>
                  )}
                </View>
              );
